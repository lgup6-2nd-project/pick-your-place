...
def add_je_to_dong_name(dong_name: str) -> str:
    # 동명에서 숫자 앞에 '제'를 넣음 (예: 창신1동 → 창신제1동)
    import re
    # '동' 직전 숫자 앞에 '제' 넣기 (숫자가 여러개여도 모두 적용 가능)
...
def main():
    ...
    output_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../data/processed/bank__processed.csv"))
    os.makedirs(os.path.dirname(output_path), exist_ok=True)  # 없으면 폴더 생성
