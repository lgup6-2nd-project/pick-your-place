...
def mapping_process(df):
    ...
    # 지번주소 → 자치구명, 법정동명
    df[['gu_name_tmp', 'dong_name']] = df['jibun_address'].apply(
        lambda addr: pd.Series(extract_gu_and_dong(addr))
    )

    # 자치구 + 동명 → 코드 매핑 (법정동이면 행정동으로 자동 매핑됨)
    df[['gu_code', 'dong_code']] = df.apply(
        lambda r: pd.Series(get_gu_dong_codes(r['gu_name_tmp'], r['dong_name'])), axis=1
    )

    # gu_name 교체
    df['gu_name'] = df['gu_name_tmp']
